  ...
  try {
    ...
    if (!parentDirId) {
      ...
      if (!rootDirectory) {
        return res
          .status(404)
          .json({ status: false, message: 'Root directory not found' })
      }
      parentDirectoryId = rootDirectory._id
    } else {
      parentDirectoryId = toObjectId(parentDirId)
    }
...
const deleteDirectoryController = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: 'Directory deleted successfully' })
  } catch (error) {
    next(error)
  }
}
...
const renameDirectoryController = async (req, res, next) => {
  try {
    ...
    if (!isDirectoryExist) {
      return res
        .status(404)
        .json({ status: false, message: 'Directory not found' })
    }
    const directory = await DirectoryModel.findByIdAndUpdate(
      toObjectId(id),
      { name },
      { new: true }
    )
